/**
 * 返回当前断点的滚动容器：移动端(≤768px)是根滚动器，桌面是 .main-content。
 *
 * Why：iOS Safari/Chrome 只在 document（根滚动器）滚动时才收起底部地址栏（全屏化）；
 * 壳 100dvh + overflow:hidden 锁死 + 内层滚动的结构永不触发。移动端滚动已还给
 * body（见 layout.css 移动端媒体查询），JS 滚动必须跟着走根滚动器；桌面滚动区是
 * .main-content（.main 本体 overflow:hidden 不滚，main-top 也不是滚动区）。
 *
 * 约定：所有 JS 滚动（回顶 / 恢复位置 / scrollTop 读写的持久化监听）一律走这里，
 * 禁止直接 querySelector 滚动容器。注意根滚动器的 scroll 事件派发在
